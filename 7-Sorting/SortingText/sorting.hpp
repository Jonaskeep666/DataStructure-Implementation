...
int listMerge(T *a,int p,int q, int n){
    // Run 1(a[p]~a[q-1])、Run 2(a[q]~a[n])
    int r1=p,r2=q,i=p;
    T *ra = new int[n+1];       //注意1：「必須」使用 ra 暫時存值
    while(r1<q && r2<=n){       //注意2：當r1、r2都沒有超出「該Run的邊界」時
        if(a[r1]<=a[r2]){ 
            ra[i]=a[r1];
            r1++; i++;
    ...
    //注意3：Update a from ra
    for(int j=p;j<=n;j++) a[j]=ra[j];
    delete[] ra;
    return p;
}

template <class T>
int rMergeSort(T *a,int left,int right){
    if(left >= right) return left;      //終止條件1：切到剩自己，回傳自己

    int mid = (left+right)/2;           //注意：永遠分成兩半，直到切到剩自己
    int r1 = rMergeSort(a,left,mid);    //Merge Sort 左半
    int r2 = rMergeSort(a,mid+1,right); //Merge Sort 右半
    return listMerge(a,r1,r2,right);    //終止條件2：合併左右兩半，回傳Run1開頭
}
